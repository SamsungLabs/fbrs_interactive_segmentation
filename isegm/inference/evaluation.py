...
def evaluate_dataset(dataset, predictor, oracle_eval=False, **kwargs):
    ...
    for index in tqdm(range(len(dataset)), leave=False):
        ...
        if oracle_eval:
            gt_mask = mx.nd.array(sample['instances_mask'], dtype='float32')
            gt_mask = mx.nd.expand_dims(mx.nd.expand_dims(gt_mask, axis=0), axis=0)
...
def evaluate_sample(image_nd, instances_mask, predictor, max_iou_thr,
    ...
    with mx.autograd.pause(), mx.autograd.predict_mode():
