...
class ZoomIn(BaseTransform):
    ...
    def transform(self, image_nd, clicks_lists, clicks_maps=None):
        ...
        if self._prev_probs is not None:
            current_pred_mask = (self._prev_probs > self.prob_thresh).asnumpy()[0, 0]
    ...
    def inv_transform(self, prob_map):
        ...
        prob_map = mx.nd.contrib.BilinearResize2D(prob_map,
                                                  height=rmax - rmin + 1, width=cmax - cmin + 1)

        new_prob_map = mx.nd.zeros_like(self._prev_probs) if self._prev_probs is not None else prob_map
...
def get_roi_image_nd(image_nd, object_roi, target_size):
    ...
    with mx.autograd.pause():
        ...
        roi_image_nd = mx.nd.contrib.BilinearResize2D(roi_image_nd, height=new_height, width=new_width)
