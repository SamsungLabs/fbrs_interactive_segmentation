...
class Crops(BaseTransform):
    ...
    def transform(self, image_nd, clicks_lists, clicks_maps=None):
        ...
        self._image_crops = mx.nd.concat(*image_crops, dim=0)
        self._counts = mx.nd.array(self._counts, ctx=image_nd.context)
    ...
    def inv_transform(self, prob_map):
        ...
        new_prob_map = mx.nd.zeros(shape=(1, 1, *self._counts.shape),
                                   dtype=prob_map.dtype, ctx=prob_map.context)
        ...
        for dy in self.y_offsets:
            for dx in self.x_offsets:
                ...
                crop_indx += 1
        new_prob_map = mx.nd.broadcast_div(new_prob_map, self._counts)
